function My_dpca_plot_default(data, time, yspan, explVar, compNum, events, signif, marg)
...
elseif ndims(data) == 4 && size(data,3)==2
    ...
    % solid/dashed
    numOfStimuli = size(data, 2);
    colors = My_hsv(numOfStimuli);

    for f=1:numOfStimuli 
        plot(time, squeeze(data(1, f, 1, :)), 'color', colors(f,:), 'LineWidth', 2)
        plot(time, squeeze(data(1, f, 2, :)), 'color', colors(f,:)./2, 'LineWidth', 2)
    end
elseif ndims(data) == 4 && size(data,3)==4
    % different stimuli in different colours and binary condition as
    % solid/dashed
    numOfStimuli = size(data, 2);
    colorsdir = My_hsv(numOfStimuli);

    for f=1:numOfStimuli 
        plot(time, squeeze(data(1, f, 1, :)), 'color', colorsdir(f,:), 'LineWidth', 2)
        plot(time, squeeze(data(1, f, 2, :)), 'color', colorsdir(f,:)/sqrt(2), 'LineWidth', 2)
        plot(time, squeeze(data(1, f, 3, :)), 'color', colorsdir(f,:)/sqrt(3), 'LineWidth', 2)
        plot(time, squeeze(data(1, f, 4, :)), 'color', colorsdir(f,:)/sqrt(4), 'LineWidth', 2)
    end

%     numOfStimuli = size(data, 3);
%     numOfStimulib = size(data, 2);
%     colorsdir = plasma(numOfStimuli);
% 
%     for f=1:numOfStimuli 
%         for i_f=1:numOfStimulib
%         plot(time, squeeze(data(1, i_f, f, :)), 'color', colorsdir(f,:), 'LineWidth', 2)
%         end
%     end
