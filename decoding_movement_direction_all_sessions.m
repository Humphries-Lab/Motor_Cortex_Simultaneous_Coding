...
for isession=1:size(Sessions,2)
    ...
    shuffle=0;
    do_plot=isession==11;
    [Acc_per_cv,error_angle,error_angle_bin]=decoding_movement_direction(selected_class,selected_angle,selected_neural,threshold,colour_dir,k_fold,Nrep,shuffle,do_plot);
    ...
    subplot(2,3,6)
    hold on
    errorbar(isession,median_error(isession),std_error(isession),'.','Color',Colour_Area(isession,:))
    subplot(2,3,5)
    hold on
    ...
    shuffle=1;
    do_plot=0;
    [Acc_per_cv_sh,error_angle_sh]=decoding_movement_direction(selected_class,selected_angle,selected_neural,threshold,colour_dir,k_fold,Nrep,shuffle,do_plot);
    subplot(2,3,6)
    hold on
    errorbar(isession,median(error_angle_sh),std(error_angle_sh),'.','Color',colour_shuffle)
